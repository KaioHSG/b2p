...
try {
    Write-Host "`nChecking GitHub for available binaries..." -ForegroundColor Gray
    $releases = Invoke-RestMethod -Uri "$llvmMinGwApiUrl/releases?per_page=5" -Headers @{"User-Agent"=$ua} -ErrorAction Stop
    ...
    if (-not $asset) { throw "No compiled binaries found in recent releases." }
