...
if ($isRemote) {
    ...
    $dataDir = Join-Path $baseDir "mingw-builds-binaries Installer Data"
}
...
try {
    Write-Host "Checking GitHub for available binaries..." -ForegroundColor Gray
    $releases = Invoke-RestMethod -Uri "$minGwW64BuildsApiUrl/releases?per_page=5" -Headers @{"User-Agent"="PowerShell-MinGW-W64-Installer"} -ErrorAction Stop
    
    $asset = $null
    $searchTerms = $buildName -split "-"

    foreach ($rel in $releases) {
        $asset = $rel.assets | Where-Object { 
            $name = $_.name
            $match = $name -like "$architecture-*.7z"
            foreach ($term in $searchTerms) {
                if ($name -notlike "*$term*") { $match = $false }
            }
            $match
        } | Select-Object -First 1

        if ($asset) { 
            Write-Host "Found build in release: $($rel.name)" -ForegroundColor Cyan
            break 
        }
    }
    
    if (-not $asset) { throw "No compiled binaries found in the last 5 releases." }
    
    $fileName = $asset.name
    $downloadUrl = $asset.browser_download_url
    Write-Host "File identified: $fileName" -ForegroundColor Gray
